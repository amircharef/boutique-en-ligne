...
import { Clock, CheckCircle2, Truck, X } from "lucide-react";
...
const columns = [
  { key: "new", label: "Nouveau", icon: Clock, nextStatus: "confirmed", nextLabel: "Confirmer" },
  {
    key: "confirmed",
    label: "Confirmé",
    icon: CheckCircle2,
    nextStatus: "shipped",
    nextLabel: "Marquer expédié",
  },
  {
    key: "shipped",
    label: "Expédié",
...
export function OrdersBoard({ orders }: { orders: OrderData[] }) {
  ...
  useEffect(() => {
    intervalRef.current = setInterval(() => router.refresh(), 8000);
    return () => {
